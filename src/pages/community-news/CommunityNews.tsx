...
import { fetchCommunityNews } from "./services/community.service.ts";
...
const CommunityNews: FC<CommunityNewsProps> = () => {
     const [paginateModel, setPaginateModel] = useState<FilterOptionsModel>({
          offset: 0,
          limit: 10,
          selects: ["title", "description", "content", "author"],
          equals: [],
          sort: [],
          relations: [],
     });

     const { data: communityNewsList } = useQuery(
          ["find-community-news", paginateModel],
          () => fetchCommunityNews(paginateModel),
          {
     ...
     const [selectedCommunityNews, setSelectedCommunityNews] = useState<CommunityNewsResponseModel>();
          ...
          <>
               <div className="section relative z-0 py-16 md:pt-32 md:pb-20 bg-gray-50">
                    <div className="container xl:max-w-6xl mx-auto px-4">
                         ...
                         {communityNewsList?.items.length === 0 ? (
                              <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} />
                         ) : (
                              <>
                                   <div className="flex flex-wrap flex-row -mx-4">
                                        {communityNewsList?.items.map((item: CommunityNewsResponseModel) => (
                                             <div
                                   ...
                                   <div className="mt-10">
                                        <Pagination
                                             defaultCurrent={1}
                                             total={communityNewsList?.totalItems}
                                             onChange={handlePagination}
                                             pageSize={paginateModel.limit}
                                             current={communityNewsList?.currentPage}
                                             showSizeChanger
                                             showTotal={(total, range) =>
                                                  `${range[0]}-${range[1]} of ${total} Community News`
                                             }
                                        />
                                   </div>
                              </>
               ...
               {isModalOpen && (
                    <MoreDetailsModal
                         selectedCommunityNews={selectedCommunityNews}
